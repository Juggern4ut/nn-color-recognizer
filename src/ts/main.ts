...
const sessionsSpan = document.querySelector("#sessions") as HTMLElement;
...
const brain = new NeuralNetwork(3, 5, 3);
...
const nextColor = () => {
  ...
  const target = complementryRGBColor(
    testValues[0] * 255,
    testValues[1] * 255,
    testValues[2] * 255
  );
...
const calculateError = (a: number[], b: number[]): number => {
 ...
 */
const complementryRGBColor = (r: number, g: number, b: number): number[] => {
 ...
 */
const setBackgroundColor = (el: HTMLElement, colors: number[]) => {
